...
library (readxl)
eolicas <- read_excel("Datos trabajo Tema_5.xlsx", sheet ="FORMAJ_EMPLEADOS_RENFIN")
...
library(dplyr)
originales2 <- eolicas %>%
  select(FORMAJ, EMPLEADOS, RENFIN)
summary (originales2)
...
originales2 %>% filter(is.na(FORMAJ) |
                         is.na(EMPLEADOS)|
                         is.na(RENFIN)) %>%
  select(FORMAJ, EMPLEADOS, RENFIN)  
...
modelo_indep <- MASS::loglm(~ FORMAJ + EMPLEADOS + RENFIN,
...
modelo_sat <- MASS::loglm(~ FORMAJ * EMPLEADOS * RENFIN,
                          data= tab.originales2)
