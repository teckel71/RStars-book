...
Primero llamamos al *data frame* a partir de cuyos datos vamos a generar la tabla, "datos". Con el operador *pipe* `%>%`, ligamos los datos del *data frame* al diseño de la tabla realizado con la función `kable()` de `{knitr}`. `kable()` tiene diversos argumentos, entre los que destacan:
...
-   **`font_size =`** : este argumento numérico se refiere al tamaño de los caracteres, lo cual es importante a la hora de que una tabla "quepa" en un documento de determinada anchura.
...
La función `seq()` genera una secuencia de *k + 1* puntos equidistantes entre el valor mínimo y el máximo de SALARIO, que son los extremos de los *k* intervalos. Luego, `breaks[-length(breaks)]` son los extremos inferiores y `breaks[-1]` los extremos superiores, y su promedio es la marca de clase.
...
El salario medio de los trabajadores de *Shuttlepod Movers* es de 16,04 cientos de PAVOs (1.604 PAVOs). Esto nos da un primer indicador del **nivel salarial típico** de la plantilla: si la masa salarial se repartiera igualitariamente entre todos los trabajadores, cada uno percibiría esa cantidad.
...
La mediana es 15 cientos de PAVOs. Esto significa que la mitad de la plantilla percibe 15 o menos cientos de PAVOs, y la otra mitad percibe 15 o más. Observemos que la mediana (15) es ligeramente inferior a la media (16,04), lo que sugiere la existencia de algunos salarios altos que "tiran" de la media hacia arriba.
