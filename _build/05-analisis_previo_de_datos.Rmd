...
Como ocurría con los *missing values*, el procedimiento de detección y eliminación de *outliers* se repite en muchos capítulos siguientes: un *boxplot* con `ggplot()` + cálculo de Q1 y Q3 con `quantile()` + `filter()` para listar los casos + `filter()` para eliminarlos. Para aligerar el código, el paquete `{MATrstars}` incluye la función auxiliar `explora_outliers()`, que encapsula todo el bloque anterior en una sola llamada y presenta la tabla de casos afectados con el estilo tipográfico habitual del libro (mediante `kable_rstars()`, internamente). Además, cuando los *outliers* son tan extremos que aplastan la caja del *boxplot*, la función activa automáticamente una escala pseudo-logarítmica en el eje Y para que la caja siga siendo legible sin problemas con valores negativos o nulos.
...
::: {style="text-align: center;"}
**Tabla 5.5.** Distribución de frecuencias agrupadas en intervalos de Rentabilidad Económica
:::


Como puede observarse, el bloque de código necesario para producir una tabla con el estilo tipográfico habitual del libro es notablemente extenso: una llamada a `kable()` con el contenido, seguida de `kable_styling()` para aplicar Bootstrap, y dos llamadas a `row_spec()` para el encabezado y el cuerpo. Y, además, ese mismo bloque de código —con muy ligeras variaciones— se repite cada vez que se genera una tabla. Para aligerar el código de los *scripts* del libro sin renunciar a la uniformidad tipográfica, se ha creado un pequeño **paquete propio** de R llamado `{MATrstars}` que reúne un conjunto de **funciones auxiliares** utilizadas a lo largo de los distintos capítulos. Este paquete no está disponible en CRAN; se distribuye a través de un repositorio público en GitHub ([https://github.com/teckel71/MATrstars](https://github.com/teckel71/MATrstars)) y se instala automáticamente la primera vez que se ejecuta el bloque siguiente (en las siguientes ocasiones, bastará con activarlo mediante `library(MATrstars)`):
...
Una de las funciones que contiene el paquete es `kable_rstars()`, un **envoltorio** de `kable()` y `kable_styling()` que aplica automáticamente todos los ajustes tipográficos habituales del libro: encabezado en negrita centrado, filas alternas rayadas, bordes discretos, cuerpo centrado, fuente de 11 puntos, punto como separador decimal y notación científica desactivada. El usuario solo necesita indicar lo específico de cada tabla —normalmente `caption`, `col.names` y `digits`—; el resto se encuentra ya configurado como valores por defecto (que también pueden ajustarse en caso necesario). De este modo, la tabla anterior podría haberse generado con este código, mucho más compacto:
...
Y el resultado obtenido sería, efectivamente, idéntico al de la tabla mostrada más arriba:
...
::: {style="text-align: center;"}
**Tabla 5.6.** Distribución de frecuencias agrupadas en intervalos de Rentabilidad Económica
:::


**A partir de esta tabla, tanto en el resto del capítulo como en los capítulos siguientes, la práctica totalidad de las tablas del libro se generarán con `kable_rstars()`**. La ficha completa de la función —argumentos, valores por defecto, ejemplos de uso y enlace al código fuente en GitHub— puede consultarse en el **Apéndice: Funciones auxiliares del paquete `MATrstars`** al final del libro.
