...
Una vez preparados los datos, vamos a presentar los grupos de naves y sus rendimientos energéticos medios. Para ello, diseñaremos una tabla con la función `kable_rstars()` del paquete `{MATrstars}`, un envoltorio de las funciones `kable()` de `{knitr}` y `kable_styling()` de `{kableExtra}` que aplica automáticamente el estilo tipográfico habitual del libro (encabezado en negrita centrado, filas alternas rayadas, bordes discretos, cuerpo centrado). El paquete `{MATrstars}` reúne funciones auxiliares utilizadas a lo largo de los distintos capítulos; los detalles y la descripción completa de cada función se recogen en el **Apéndice: Funciones auxiliares del paquete `MATrstars`**. Para construir la tabla, hemos de crear anteriormente un pequeño *data frame*, llamado por ejemplo "tablamedias", en el que cada caso o fila sea uno de los grupos en que queda dividida la muestra a partir de los niveles del factor "ASTILLERO", y que contenga tres variables: el ASTILLERO de cada grupo o submuestra, su número de casos contenidos (variable "observaciones"), y los respectivos IRE medios (variable "media"):
