...
**Figura 10.1.** [Galaxias que recorrer...]{.smallcaps}
:::
...
## ![](figuras/book.svg){.hicon} Tablas de contingencia, y asociación entre dos atributos o factores.

### ![](figuras/star.svg){.hicon} ![](figuras/pie-chart.svg){.hicon} El entramado comercial del cosmos.
...
(*Environment*). Además, se cargan los paquetes que harán falta para
ejecutar el código:
...
### ![](figuras/star.svg){.hicon} ![](figuras/pie-chart.svg){.hicon} La sospecha institucional (GALAXIA XFJUR).
...
### ![](figuras/star.svg){.hicon} ![](figuras/pie-chart.svg){.hicon} ¿Se Refleja la estructura jurídica en la renovación de la flota? (FJUR × EFLO).
...
## ![](figuras/book.svg){.hicon} Modelos logaritmico-lineales aplicados a tablas de contingencia.
...
### ![](figuras/star.svg){.hicon} ![](figuras/pie-chart.svg){.hicon} La complejidad del sistema de relaciones (GALAXIA X FJUR X EFLO).
...
generar_solucion <- function(modelo) {
  # Detectar formato de salida actual (html, docx, pdf, ...)
  tipo_output <- knitr::opts_knit$get("rmarkdown.pandoc.to")
  ...
  # Formatear la tabla de validación usando kable
  if (tipo_output == "html") {
    independencia_valida_tab <- tabla_informacion %>%
      kable(
            format    = "html",
    ...
    # Otras salidas (pdf, etc.): tabla sencilla genérica
    independencia_valida_tab <- tabla_informacion %>%
      kable(
            col.names = c("Prueba", "Estadístico",
                          "Grados Libertad", "P-valor"))
...
Una vez identificados los valores ausentes, se depura el *data frame*
conservando únicamente las filas completas. En el mismo paso, se
**recodifican los nombres de las categorías de GALAXIA** para
abreviarlos y facilitar tanto la visualización de las tablas como la
interpretación de resultados en gráficos y salidas de modelos.
Finalmente, se convierten todas las variables a **formato factor**,
asegurando que R trate correctamente a cada una como un atributo
categórico. Este flujo prepara un conjunto de datos consistente,
depurado y listo para construir la tabla de contingencia tridimensional
y avanzar hacia el ajuste de los modelos log-lineales.
...
# Detectar formato de salida actual
tipo_output <- knitr::opts_knit$get("rmarkdown.pandoc.to")
...
::: {style="text-align: center;"}
**Tabla 10.5.** Combinaciones con 0 detectadas (antes de imputar)
:::
...
# Mostrar nota de imputación
cat("\nNOTA IMPUTACIÓN: ", attr(tab_use, "nota_imputacion"), "\n")

# Mostrar detalle de celdas imputadas (si aplica)
if (!is.null(attr(tab_use, "zeros_imputados"))) {
  zeros_imputados <- attr(tab_use, "zeros_imputados")
...
::: {style="text-align: center;"}
**Tabla 10.6.** Combinaciones imputadas (0 → 1) para estimar modelo
:::


En nuestro caso, solo se ha detectado una combinación de categorías o
niveles de los factores con 0 observaciones; la correspondiente a
GALAXIA = "Pequeña Nube de Magallanes", FJUR = "CG" (cooperativa) y EFLO
= "RENOVADA". A esta combinación se le imputa una frecuencia "ficticia"
con el fin de que el modelo sea estimable, que es la única diferencia
que tendrá la tabla "tab_use" con respecto a la tabla original, "tab0".
Huelga decir que en la estimación será "tab_use" la tabla utilizada como
base.
...
incluye varias interacciones. Para simplificar su uso, hemos definido
dos funciones auxiliares, que se detallaron al comienzo del *script*:
...
**Nota técnica:** la función genérica `step()` localiza, para cada
submodelo que prueba, la tabla de datos original contando internamente
cuántas llamadas hay por encima en la pila de ejecución
(`eval.parent()`). Esto funciona sin problema en una sesión interactiva,
pero es **frágil cuando el documento se compila a través de varias capas
de funciones** (por ejemplo, al llamar a `bookdown::render_book()` desde
dentro de una función como `build_book()`), ya que entonces cambia el
número de llamadas apiladas y `step()` deja de encontrar correctamente
los datos, provocando un error solo al renderizar el libro. Para
evitarlo, implementamos manualmente el mismo algoritmo de selección
*backward* por AIC (respetando la jerarquía de los términos), pero
reajustando cada submodelo con una llamada directa y autocontenida a
`MASS::loglm()`, sin depender de `update()` ni de la pila de llamadas:

```{r, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
# Selección backward por AIC, robusta frente al contexto de ejecución
# (consola, knitr, bookdown, funciones envolventes, etc.).
...
step_loglm_backward <- function(formula, data, trace = TRUE, steps = 1000) {
  ...
  for (i in seq_len(steps)) {
    ...
    aics <- vapply(terminos, function(term) {
      nueva_formula <- update.formula(formula_actual, paste("~ . -", term))
    ...
    formula_actual <- update.formula(formula_actual,
...
## ![](figuras/book.svg){.hicon} Análisis de correspondencias.
...
### ![](figuras/star.svg){.hicon} ![](figuras/pie-chart.svg){.hicon} De la asociación a su mapa: Galaxia y Forma Jurídica revisitadas.
...
::: {style="text-align: center;"}
**Tabla 10.17.** Empresas TMI: Galaxia y Forma Jurídica.
:::
