...
En el Global Environment vemos que se ha creado el objeto "Cl.ojosF", de tipo factor. Si representamos ambos objetos:
...
Obviamente, para cargar o activar un paquete, previamente debe de haber sido instalado en la máquina donde estamos trabajando. Si no hemos instalado antes los paquetes `{readxl}` y `{gtExtras}`, tendremos que instalarlos siguiendo el procedimiento descrito en la sección de *Paquetes* del capítulo anterior (sección 1.7), o nos dará error.
...
Vemos que ya no aparece "NOMBRE" como variable, y en el *Global Environment* ya aparece el data frame "interestelar_100" con 100 observaciones (casos), pero con 28 variables (una menos).
...
Vamos a suponer que trabajamos dentro del proyecto que hemos creado previamente, de nombre "explora" (ver capítulo 1), y que seguimos con el script "explora_rstars.R", con el que importamos los datos del archivo de Microsoft® Excel® "interestelar_100.xlsx", que contenía información sobre 104 empresas de transporte de mercancías a escala interestelar.
...
Vamos a continuar desde este punto. A continuación, cargaremos el paquete `{dplyr}` (si no está instalado, instálalo como se indicó en la sección 1.7):
...
Podemos verificar que, en el *Global Environment,* el *data frame* ha pasado a tener una variable menos (27), ya que hemos eliminado la variable FJUR. Es decir, con el guión "-" se pueden eliminar directamente variables de un *data frame*.
...
Como no hemos asignado el resultado de la función a ningún "nombre", R simplemente muestra el resultado en pantalla; pero no guarda ningún objeto en el *Global Environment*. Si asignamos un `select()` a un "nombre", se creará un *data frame* con ese nombre, y las variables seleccionadas:
...
### ![](figuras/key.svg){.hicon} El operador *pipe*.
...
### ![](figuras/key.svg){.hicon} Seleccionando casos por posición: `slice()`.
...
Este mismo código puede escribirse de un modo más legible utilizando el operador *pipe* `%>%` que ya presentamos al comienzo de la sección:
...
R cuenta con un **formato propio de datos**, que se traduce en archivos de extensión "RData", y que puede incluir cualquier objeto de R. Vamos a exportar el *data frame* "interestelar_100" como el archivo de datos de R "interestelar_100.RData". Posteriormente, borraremos el *data frame* del *Global Environment* y recuperaremos los datos cargando ese archivo "interestelar_100.RData".
