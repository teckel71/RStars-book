...
library(readxl)
datos <- read_excel("Datos_ejercicio_01_tema_03.xlsx", sheet = "HURTADO JAVIER", na = "n.d.")
...
datos_so <- datos %>%
  filter(MAHALANOBIS <= Q3M + 1.5*IQR(MAHALANOBIS) &
         MAHALANOBIS >= Q1M - 1.5*IQR(MAHALANOBIS))
datos_so <- datos_so %>% select(-MAHALANOBIS)

# Correlaciones.

library (GGally)

corr_plot_so <- ggpairs(datos_so, 
...
componentes <- prcomp (datos_so, scale=T)
...
muestra <- select(datos, everything())
muestra <- muestra %>%
  mutate (MAHALANOBIS = mahalanobis(cbind(Variable_1, Variable_2, Variable_3, Variable_4),
                                    center = colMeans(select(., Variable_1, Variable_2, Variable_3, Variable_4)),
                                    cov = cov(select(., Variable_1, Variable_2, Variable_3, Variable_4))))
library (ggplot2)
ggplot(data = muestra, map = (aes(y = MAHALANOBIS))) +
  geom_boxplot(fill = "red") +
  ggtitle("DISTANCIA DE MAHALANOBIS", subtitle = "CASOS") +
  ylab("MAHALANOBIS")
