...
Ahora, suponemos que queremos visualizar las variables del *data frame* "interestelar_100": ACTIVO, FPIOS, LIQUIDEZ, MARGEN, SOLVENCIA y APALANCA. Para ello, ejecutaremos el código:
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
df <-select(interestelar_100, ACTIVO, FPIOS, LIQUIDEZ, MARGEN, SOLVENCIA, APALANCA)
...
Podemos comprobar cómo en el *Global Environment* hay otro objeto *data frame* llamado "interestelar_100A", con 6 variables (y los mismos 104 casos).
...
#definir df
df<-filter(interestelar_100, RES >= 500)
...
Se pueden incluir varias condiciones en un mismo filtro. Por ejemplo, vamos a construir un nuevo *data frame* llamado "interestelar_100B" con las empresas que posean un resultado mayor o igual a 500 miles de PAVOs y una rentabilidad económica (variable RENECO) inferior al 40%:
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
interestelar_100B <-filter(interestelar_100, RES >= 50000 & RENECO < 6)
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
df<-arrange(interestelar_100, RENECO)
# Número de columnas por bloque
columnas_por_bloque <- 3
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
df<-arrange(interestelar_100, desc(RENECO))
# Número de columnas por bloque
columnas_por_bloque <- 3
...
```{r, eval=TRUE, echo=FALSE, message=FALSE}
df<-arrange(interestelar_100, EFLO, desc(RENECO))
# Número de columnas por bloque
columnas_por_bloque <- 3
...
Podríamos interpretar la línea de código así: "asigna al *data frame* "interestelar_100" sus propios datos, después (`%>%`) crea la variable DIM con la función `cut()` y añádela a "interestelar_100".
...
En cuanto a las variables que componen la base de dato, son las siguientes:
...
-   **ACTIVO.** Valor total de los recursos de la empresa (miles de PAVOs). Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 17.55, 'q1': 50.53, 'median': 156.315, 'mean': 241.40656666666666, 'q3': 227.01749999999998, 'max': 2966.52}. Número estimado de outliers: 45.

-   **FPIOS.** Fondos propios de la empresa. Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 8.67, 'q1': 22.53, 'median': 57.43, 'mean': 111.04200666666667, 'q3': 108.4075, 'max': 1453.45}. Número estimado de outliers: 23.

-   **ING.** Ingresos generados por la actividad principal de la empresa. Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 19.4, 'q1': 42.005, 'median': 103.47, 'mean': 218.3861666666667, 'q3': 240.9725, 'max': 2471.54}. Número estimado de outliers: 23.

-   **MARGEN.** Margen operativo como porcentaje sobre los ingresos. Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 12.43160707469143, 'q1': 48.094728067689005, 'median': 57.449284410494265, 'mean': 60.159512199694774, 'q3': 73.6097083921985, 'max': 96.48871372268005}. Número estimado de outliers: 0.

-   **RES.** Resultado del ejercicio (beneficio neto). Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 9.62, 'q1': 25.315, 'median': 59.22, 'mean': 120.58346666666667, 'q3': 128.1775, 'max': 1561.93}. Número estimado de outliers: 28.

-   **SOLVENCIA.** Capacidad de la empresa para cubrir sus obligaciones financieras a largo plazo. Tipo de datos: numérico (real o entero). NAs: 0. Distribución/aproximación: {'min': 123.07, 'q1': 164.7025, 'median': 182.06, 'mean': 185.7181, 'q3': 199.415, 'max': 325.32}. Número estimado de outliers: 10.

ACTCOR

Descripción: Activo corriente.

Tipo de datos: numérico (real o entero)

NAs: 0

Observaciones corruptas: 0

Distribución/aproximación: {'min': 6.81, 'q1': 17.450000000000003, 'median': 24.005000000000003, 'mean': 59.78711333333333, 'q3': 55.635, 'max': 889.956}

Número estimado de outliers: 36

PASCOR

Descripción: Pasivo corriente.
...
Distribución/aproximación: {'min': 6.66, 'q1': 20.221875, 'median': 59.22375, 'mean': 97.77342, 'q3': 100.558125, 'max': 1143.81}

Número estimado de outliers: 25

LIQUIDEZ

Descripción: Cociente entre ACTCOR y PASCOR.
...
Distribución/aproximación: {'min': 0.2253031527890057, 'q1': 0.5258905294495814, 'median': 0.7055401985223915, 'mean': 0.7017208938995255, 'q3': 0.8363331747719515, 'max': 3.1175}

Número estimado de outliers: 7

APALANCA

Descripción: Nivel de apalancamiento (Pasivo total / Fondos propios).
...
Distribución/aproximación: {'min': 44.38, 'q1': 100.5925, 'median': 121.86, 'mean': 132.66719999999998, 'q3': 154.55, 'max': 433.55}

Número estimado de outliers: 13

RENECO

Descripción: Rentabilidad económica.
...
Distribución/aproximación: {'min': 24.0818338673897, 'q1': 44.11017192391375, 'median': 50.322615496080985, 'mean': 51.05483246133457, 'q3': 57.744859307359306, 'max': 93.8474791439971}

Número estimado de outliers: 4

RENFIN

Descripción: Rentabilidad financiera.
...
Distribución/aproximación: {'min': 51.38759229398286, 'q1': 95.62038443747954, 'median': 113.04759181292664, 'mean': 116.1380407827033, 'q3': 131.1379538686389, 'max': 262.245027239326}
...
Distribución/aproximación: {'min': 7.0, 'q1': 22.75, 'median': 30.0, 'mean': 36.28666666666667, 'q3': 44.0, 'max': 106.0}

Número estimado de outliers: 16

FJUR

Descripción: Forma jurídica de la empresa.

Tipo de datos: carácter/categórico

NAs: 0

Observaciones corruptas: 0

Distribución/aproximación: {'SLG': 131, 'SAG': 87, 'CG': 82}

Número estimado de outliers: No aplica

FLOTA

Descripción: Número de cargueros espaciales.
...
Distribución/aproximación: {'min': 3.0, 'q1': 15.0, 'median': 24.0, 'mean': 26.143333333333334, 'q3': 38.0, 'max': 50.0}

Número estimado de outliers: 0

COSTOP

Descripción: Costos operativos.
...
Distribución/aproximación: {'min': 0.9800000000000004, 'q1': 15.305, 'median': 31.195, 'mean': 97.8027, 'q3': 105.1425, 'max': 1422.3}
...
Distribución/aproximación: {'min': 1.038287729317929, 'q1': 6.155582323293937, 'median': 11.59993993880682, 'mean': 33.7481042038456, 'q3': 34.328503663564, 'max': 1087.195548557454}

Número estimado de outliers: 29

IMD

Descripción: Gasto en I+D.
...
Distribución/aproximación: {'min': 0.4779202472567501, 'q1': 1.409149294544295, 'median': 4.7002794818005755, 'mean': 15.20343463327611, 'q3': 18.239778326202334, 'max': 202.4779489786356}

Número estimado de outliers: 40

EFLO

Descripción: Edad media de la flota (categorías).
...
Descripción: Número de rutas atendidas.
...
Distribución/aproximación: {'min': 36.0, 'q1': 176.0, 'median': 310.0, 'mean': 365.58, 'q3': 436.5, 'max': 3138.0}

Número estimado de outliers: 12

FROM

Descripción: Inspiración temática de la empresa.
...
Descripción: Índice de digitalización.
...
Distribución/aproximación: {'min': 0.3378663961065547, 'q1': 8.497779899152183, 'median': 15.67220425672831, 'mean': 17.23429190850527, 'q3': 23.848371427139387, 'max': 60.83251420063471}

Número estimado de outliers: 4

IDIVERSE

Descripción: Índice de diversificación.
...
Distribución/aproximación: {'min': 0.0, 'q1': 11.654577691811731, 'median': 21.165377176015475, 'mean': 22.20967117988394, 'q3': 32.39039329464862, 'max': 78.29787234042553}

Número estimado de outliers: 2

IFIDE

Descripción: Índice de fidelización.
...
Distribución/aproximación: {'min': 5.260711052386895, 'q1': 36.927508537079916, 'median': 39.26811099440512, 'mean': 39.00159967860573, 'q3': 40.431855833880604, 'max': 79.15382971495461}

Número estimado de outliers: 33

GMEDRUT

Descripción: Distancia media de las rutas (años luz).
...
Distribución/aproximación: {'min': 0.005197255574614065, 'q1': 0.11182702558635398, 'median': 0.2235808625336927, 'mean': 0.31892273948109423, 'q3': 0.44811473004320435, 'max': 1.550434782608695}

Número estimado de outliers: 10

EFLO_FACTOR

Descripción: Factor derivado de la edad media de la flota.
...
Distribución/aproximación: {'min': 0.8, 'q1': 0.8, 'median': 1.0, 'mean': 0.9426666666666665, 'q3': 1.0, 'max': 1.2}

Número estimado de outliers: 0

GALAXY_FACTOR

Descripción: Factor de distancia intergaláctica.
...
Distribución/aproximación: {'min': 1.0, 'q1': 1.0, 'median': 2.5, 'mean': 2.8116666666666665, 'q3': 4.5, 'max': 5.0}

Número estimado de outliers: 0

DIST

Descripción: Distancia operada globalmente.
...
Distribución/aproximación: {'min': 0.1512896531277795, 'q1': 12.605721150251313, 'median': 44.228875683782675, 'mean': 133.68609306654542, 'q3': 165.29212487633055, 'max': 2151.641799196787}

Número estimado de outliers: 26

BMAL

Descripción: Beneficio medio por año luz.
...
Distribución/aproximación: {'min': 24.1346865503376, 'q1': 466.11372981210417, 'median': 1491.4160131662015, 'mean': 6084.856716904903, 'q3': 4368.46087373694, 'max': 262555.8151022698}
